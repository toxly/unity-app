...
namespace fastJSON
{
    internal class Getters
    {
		...
		public MethodInfo getMothedInfo;
		public FieldInfo  getFieldInfo;

		public object getValue(object pThis)
		{
			if (getMothedInfo != null) {
				return getMothedInfo.Invoke(pThis,new object[]{});
			} else if (getFieldInfo != null) {
				return getFieldInfo.GetValue(pThis);
			}
    ...
    internal class myPropInfo
    {
		...
		{
			if (getMethod != null) {
				return getMethod.Invoke(pThis,new object[]{});
			} else if (getFieldInfo != null) {
				return getFieldInfo.GetValue(pThis);
			}
		...
		public void setValue(object pThis, object value)
		{
			if (setMethod != null) {
				setMethod.Invoke(pThis,new object[1]{value});
			} else if (setFieldInfo != null) {
				setFieldInfo.SetValue(pThis,value);
			}
        ...
        public Dictionary<string, myPropInfo> Getproperties(Type type, string typename, bool customType)
        {
            ...
            else
            {
                ...
                foreach (PropertyInfo p in pr)
                {
					...
					d.setMethod = Reflection.CreateSetMethod(type, p);
					if (d.setMethod != null)
                        d.CanWrite = true;
                    d.getMethod = Reflection.CreateGetMethod(type, p);
                ...
                foreach (FieldInfo f in fi)
                {
                    ...
                    {
						d.setFieldInfo = f;
						if(d.setFieldInfo!=null)
                       		d.CanWrite = true;
						d.getFieldInfo = f; //Reflection.CreateGetField(type, f);
...
//        }
		...
		internal static MethodInfo CreateSetMethod(Type type, PropertyInfo propertyInfo)
        {
...
//            return (GenericGetter)dynamicGet.CreateDelegate(typeof(GenericGetter));
 //       }
		...
		internal static MethodInfo CreateGetMethod(Type type, PropertyInfo propertyInfo)
        {
...
//            return (GenericGetter)getter.CreateDelegate(typeof(GenericGetter));
        ...
        internal Getters[] GetGetters(Type type, bool ShowReadOnlyProperties, List<Type> IgnoreAttributes)//JSONParameters param)
        {
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                MethodInfo g = CreateGetMethod(type, p);
                if (g != null)
					getters.Add(new Getters { getMothedInfo = g, Name = p.Name, lcName = p.Name.ToLower() });
            }
            ...
            foreach (var f in fi)
            {
                ...
                if (f.IsLiteral == false)
                {
                    ...
                    //if (g != null)
                     getters.Add(new Getters { getFieldInfo = f, Name = f.Name, lcName = f.Name.ToLower() });
                }
            }
            val = getters.ToArray();
